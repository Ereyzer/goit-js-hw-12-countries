...
const refs ={
    ...
    async function getInputValue(e) {
        ...
        refs.list.innerHTML = '';
        try {
            const response = await fetchCountries(searchQuery);
            if(response.length > 10)throw new Error('We have more ten results. Please enter more characters!');
            if(response.status === 404)throw new Error('no EXIST');
            makeList(response)
            if (response.length === 1) {
                refs.list.innerHTML ='';
                return await makeCards(response);
             }
             return response;
        } catch (error) {
            errorEntries(error);
    ...
    function onItemsClick(e) {
        const choiceCountry = e.target.textContent
        countries = [countries.find(country => country.name === choiceCountry)]
        makeCards(countries);
        refs.input.value = '';
        refs.list.innerHTML = '';
        return;
